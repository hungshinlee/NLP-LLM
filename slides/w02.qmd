...
## Read it first: the paragraph every number today is measured on

::: {.gen .read}
清晨六點半，城市還沒完全醒來。巷口的早餐店已經亮起燈，老闆一邊煎蛋餅，一邊和熟客聊著昨晚的颱風消息。捷運站裡，通勤的人們低頭滑著手機，有人在讀新聞，有人在回訊息，也有人只是閉著眼睛，想多偷幾分鐘的睡眠。公車站牌旁，一位老先生慢慢地把雨傘收好，抬頭看了看天空：雲層很厚，但風已經小了。對許多人來說，這只是再平凡不過的星期三；可是對剛搬來這座城市的學生而言，每一個轉角都像是一個還沒讀完的句子，等著他們自己去補上後面的字。
:::

::: {.ask}
Before any number: how many tokens would a modern model spend on this paragraph? Write one guess down.
:::

::: {.footer}
209 characters · 627 UTF-8 bytes · written for this course; in no model's training data
:::
...
| Gemma 4 26B-A4B (multilingual, ~260k vocabulary) | 164 | 0.785 | 3.82 | 1.16 |
| Mistral 7B v0.3 (SentencePiece, 32k, English-heavy) | 272 | 1.301 | 2.31 | 1.93 |
| ByT5 (every byte is a token) | 627 | 3.000 | 1.00 | 4.45 |

::: {.claim}
The same text costs $\mathbf{1.9\times}$ more on one modern model than another — and the byte model marks the ceiling: $T \leq B$, always.
:::
...
| text | GPT-2 | o200k | Llama 3 | Qwen3.8 | Gemma 4 | Mistral | Breeze | bert-zh | ByT5 |
|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 繁中 (209 chars) | | | | | | | | | 627 |
| the same, in English | | | | | | | | | |
| 簡體 (OpenCC) | | | | | | | | | |
...
::: {.small}
Why: NFC stores `â` as one code point, U+00E2, $2$ bytes. NFD stores `a` + a combining circumflex, U+0061 U+0302, $1 + 2$ bytes. Each tone mark costs $+1$ character and $+1$ byte; four marks take the Tâi-lô line from $22$ to $26$.
:::
...
- The price: sequences $3.8$–$4.5\times$ longer than a modern subword model on Chinese ($627$ vs $164$ and $141$)
:::
:::
...
::: {.footer}
Condition (a) from W1 p29 · fertility rows for 台語 and 客語 on the live table
:::
...
| input | after NFKC | bytes, before $\to$ after |
|:---|:---|:---|
| ＡＢＣ１２３ (full-width) | ABC123 | $18 \to 6$ |
| ①②③ | 123 | $9 \to 3$ |
| ㎏ | kg | $3 \to 2$ |
| café (NFC) | café | $5 \to 5$ |
...
::: {.footer}
Python `unicodedata.normalize` · Qwen3.8 applies NFC (source 2026-09-17; `inspect` 2026-09-20)
:::
...
| Qwen (2023–) | `\p{N}` — one digit per token | `1` `2` `3` `4` `5` `6` `7` | no grouping to get wrong; sequences $3\times$ longer on numbers |
...
::: {.footer}
Rules read from source; all nine tokenizers checked live 2026-09-20 (demo 1 `inspect`)
:::
...
::: {.small}
Last week's one-line conversion, in this week's notation: $\mathrm{BPB} = \dfrac{T}{B}\log_2 \mathrm{PPL} = \dfrac{\log_2 \mathrm{PPL}}{\bar{b}}$ — stage 2's score divided by stage 1's compression. Check on last week's numbers: $\log_2 12.23 / 4.45 = 0.81$, $\log_2 9.14 / 2.31 = 1.38$.
:::
...
::: {.footer}
Toy corpus: WordPiece's first merge is `i`+`d` (score $0.50$, seen $2\times$); BPE's `n`+`e` ranks 7th
:::
...
::: {.footer}
Byte counts: make_demo_w02.py §1
:::
...
::: {.footer}
Misconception 3 of 10: "a bigger vocabulary is better" · Tao et al., NeurIPS 2024 (main paper)
:::
...
::: {.footer}
Skeleton only — the fitted form and the exponent are in Tao et al. (arXiv:2407.13623)
:::
...
::: {.footer}
Shapes: GPT-3 Table 2.1 · $N_{\text{nv}} = 12\,n_{\text{layer}} d^2$ (Kaplan) · Qwen3.8: demo 1
:::
...
::: {.claim}
The same token budget buys $\mathbf{1.9\times}$ **more Chinese text** through one tokenizer than through another. Report data in bytes or characters, or the comparison is meaningless.
:::
...
| $\approx$ Chinese characters in that window ($3$ bytes each) | $\approx 48{,}600$ | $\approx 25{,}200$ | 1.93 |
...
::: {.footer}
Misconception 6 of 10 · arXiv:2402.14903 (rules: its §3) · measured on the next slide
:::
...
- Model: **Llama 3.1 8B** (`\p{N}{1,3}`: `123` `456` `7`) — **not Qwen**, whose per-digit tokenizer makes (A) and (C) the same thing
- **Qwen3 8B** as the control: per-digit tokens, so (A) and (B) should differ much less
- Thinking mode **off** — with it on, the model rewrites the digits in its own scratchpad and bypasses the tokenizer (W10)

::: {.footer}
Demo 3 · live: 20 problems per cell · rehearsal 2026-09-20: 50 per cell, both models 4-bit MLX
:::
...
|:---|:---|:---|:---|
| **ByT5** (2022) | every byte is a token; no vocabulary | sequences $3.8$–$4.5\times$ longer on Chinese (today's table) | no OOV, no normalizer, no glitch tokens; robust to noise |
...
|:---|:---|:---|:---|
| **Extend the vocabulary**, then continue pre-training | Chinese-LLaMA (+20k tokens on LLaMA); **Breeze-7B**: Mistral's 32,000 $\to$ 61,872 (+29,873 Traditional Chinese) | new embeddings start from zero — CPT compute to fill them; risk to the base model's abilities (W8: catastrophic forgetting) | Breeze's report: $\approx 2\times$ compression and $\approx 2\times$ speed on Traditional Chinese |
| **Transplant the tokenizer** | Zero-Shot Tokenizer Transfer (Minixhofer, Ponti & Vulić, 2024): a hypernetwork predicts embeddings for a new vocabulary | a small gap to the original; the paper says under 1B tokens of continued training closes most of it; validated up to 7B | swap the retina without retraining the cortex — almost |
...
- Keeping the two apart is what lets you decide **which one to invest in**: Breeze-7B invested in the second alone and reported $\approx 2\times$ gains without touching the first
