...
## Two curves explain the whole sweep {.smaller}
...
## Same prompt. A 2026 model. Live. {.smaller}
...
## Two numbers, now, on paper {.smaller}
...
| **Supervision** | Where does this capability come from? There are five possible answers — and today exactly one of them exists |
...
## Today, the third column has one entry {.smaller}
...
Everything in today's lecture — n-grams, word2vec, an LSTM language model, seq2seq — is **self-supervised from raw text**. Nothing else is on the menu yet.

::: {.claim .warn}
**W8 is where that column starts to branch.** You will not feel how large that change is unless you notice how empty the column is today.
:::
...
## What you will run: nothing {.smaller}
...
## Live: the same paragraph, two tokenizers {.smaller}
...
## "Comparable" has three conditions {.smaller}
...
## And a low perplexity does not mean good generations {.smaller}
...
## Why more data cannot fix it: Zipf and Heaps {.smaller}
...
## That attention is cross-attention, not self-attention {.smaller}
...
## And what recurrence never lost {.smaller}
