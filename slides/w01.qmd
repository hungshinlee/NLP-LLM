...
## Ten things this week said no to

| The belief | What is true | Returns |
|---|---|---|
| Lower perplexity, better text | Likelihood and sample quality are nearly independent | W14 |
| Perplexity compares across models | Denominator is tokens — compare bits per byte instead | W2 |
| Hallucination means bad data | A calibrated model hallucinates at the singleton rate | W9, W12 |
| An LLM is a large n-gram | Lookup vs. interpolation; the long tail is memorized | W7, W14 |
| Negative sampling ≈ softmax | That is NCE; the SGNS optimum is $\mathrm{PMI} - \log K$ | W12 |
| Cosine similarity is meaning | What the objective rewarded — here, shared contexts | W12 |
| A vector holds the commonest sense | Its senses are superposed in one vector | W14 |
| That attention was self-attention | Cross-attention: query and keys from two sequences | W3, W5 |
| Attention came with the Transformer | Bahdanau 2014; the Transformer only dropped recurrence | W3 |
| RNNs lost on quality | Parallelism and path length; fixed state still pays | W6 |

::: {.footer}
Misconceptions 1–10, one row per slide, in the order they appeared today · the last column is where each returns with real numbers
:::
