...
format:
  revealjs:
    ...
    incremental: false
    # inline SVG 的 fragment（2026-09-19 起，先用在算術編碼那張）：?print-pdf 時整張印在一頁，
...
{{< include assets/w01/arith.inline.qmd >}}
...
3. **Bits-per-byte comparisons come with conditions.** Four of them, four slides from now
...
## One cell, applied once per token

![](assets/w01/rnn.svg){width="100%"}

::: {.claim}
Two consequences: any length fits, because nothing in the weights depends on $t$ — and everything the model knows about the past sits in one fixed-size $\mathbf{h}_t$.
:::
...
|---|---|---|
| **1** · Lower perplexity, better text | Likelihood and sample quality are nearly independent; $n = 4$ read best and scored worse | W14 |
| **2** · Perplexity compares across models | Its denominator is tokens; compare bits per byte, on the same bytes | W2 |
| **3** · Hallucination means bad data | A calibrated model on clean data still hallucinates at about the singleton-fact rate | W9, W12 |
| **4** · An LLM is a large n-gram | Different inductive bias — lookup vs. interpolation; but the long tail is memorized | W7, W14 |
:::

::: {.footer}
Misconceptions 1–4: the ones about measurement · one row per slide, in the order they appeared · the last column is where each returns with real numbers
:::
...
| The belief | What is true | Returns |
|---|---|---|
| **5** · Negative sampling approximates softmax | That is NCE; the SGNS optimum is $\mathrm{PMI} - \log K$ | W12 |
| **6** · Cosine similarity is meaning | It is whatever the objective rewarded — here, shared contexts | W12 |
| **7** · A vector holds the commonest sense | Its senses are superposed in one vector | W14 |
| **8** · That attention was self-attention | Cross-attention: the query and the keys come from two sequences | W3, W5 |
| **9** · Attention came with the Transformer | Bahdanau 2014, Luong 2015; the Transformer only dropped the recurrence | W3 |
| **10** · RNNs lost on quality | Parallelism and path length; the fixed-size state is still an asset at inference | W6 |
:::

::: {.footer}
Misconceptions 5–10: vectors, then attention and recurrence · the last column is where each returns with real numbers
:::
...
## Next week: the unit that costs most to change after training

**Tokenization.** The design decision in the whole system that is **most expensive to fix post hoc** — not impossible, but every route back means paying for training again.

- Today ended with words that had no vector. Next week: what a model sees instead
- We build BPE by hand, and put the same Traditional Chinese paragraph through nine tokenizers to measure fertility — with the actual token counts that make perplexity incomparable
