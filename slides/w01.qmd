...
- Everything is on the course site: weekly pages, the reading list, these slides, and four Chinese guides for the two reports
...
::: {.footer}
The 16 GB card is the one you just costed · W3 opens by asking what its memory holds
:::
...
::: {.claim .ok}
Use **bits-per-byte**: the same total log-likelihood in bits, divided by the number of bytes $B$ — a property of the text. Converting takes one line: $\mathrm{BPB} = \frac{T}{B}\log_2 \mathrm{PPL}$.
:::
...
|---|---|---|---|---|
| 1 | 0.622 | 0.231 → 0.029 | −0.46 | about ×5 |
| 2 | 0.852 | 0.738 → 0.319 | −0.19 | about ×40 |
| 3 | 0.963 | 0.926 → 0.733 | −0.05 | about ×500,000 |
| 4 | **0.990** | **0.975 → 0.917** | **−0.02** | about $\times 2^{66}$ |

::: {.claim .warn}
A useful n-gram model needs n ≥ 3 — which is exactly where the decay stops. That is what "more data cannot fix it" means.
:::
...
$$\mathcal{L} = -\log\sigma(\mathbf{u}_o^\top \mathbf{v}_c) \;-\; \sum_{k=1}^{K}\log\sigma(-\mathbf{u}_{w_k}^\top \mathbf{v}_c), \qquad w_k \sim P_n$$

Push a word and its true context together; push it away from $K$ sampled non-contexts, drawn from the unigram distribution raised to the 3/4.
...
$$\frac{\partial \mathcal{L}}{\partial h_1} = \frac{\partial \mathcal{L}}{\partial h_T}\prod_{t=2}^{T}\frac{\partial h_t}{\partial h_{t-1}}, \qquad \frac{\partial h_t}{\partial h_{t-1}} = \mathrm{diag}\big(\phi'(\cdot)\big)\,W$$

So the product's norm is at most $(\gamma\,\sigma_{\max}(W))^{T-1}$, where $\gamma = \sup|\phi'|$ — 1 for tanh.
...
- **1997** (Hochreiter & Schmidhuber): $c_t = c_{t-1} + i_t \odot g_t$. Along the cell, the Jacobian is $I$ — a genuinely **additive path**
- **2000** (Gers, Schmidhuber & Cummins): add a forget gate, $c_t = f_t \odot c_{t-1} + i_t \odot g_t$. Now it is $\mathrm{diag}(f_t)$: still a product, but one the model **learns to hold near 1**. This is the LSTM in use today

::: {.claim}
LSTM: a gated additive path across **time**. Highway networks: gated, across **depth**. ResNet: the gate removed, $x_{l+1} = x_l + F(x_l)$. **That last line is the residual stream of W4.**
:::
...
**History and n-grams** · Shannon, *A Mathematical Theory of Communication*, Bell System Technical Journal, 1948 · Shannon, *Prediction and Entropy of Printed English*, Bell System Technical Journal, 1951 · Kneser & Ney, *Improved backing-off for M-gram language modeling*, ICASSP 1995 · Chen & Goodman, *An empirical study of smoothing techniques for language modeling*, Computer Speech & Language, 1999 · Bengio, Ducharme, Vincent & Jauvin, *A Neural Probabilistic Language Model*, JMLR 2003 · Liu et al., *Infini-gram*, COLM 2024 (arXiv:2401.17377) · Nguyen, *Understanding Transformers via N-gram Statistics*, NeurIPS 2024 (arXiv:2407.12034)
