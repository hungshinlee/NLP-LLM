...
`DROP_UNVERIFIED = False`——四級引用標記是逐條查證過的、Syllabus 有向學生說明，
所以原樣上站。
...
| 網站語言 | **介面與框架英文、課程內文中文**。英文：`index.qmd`、`syllabus.qmd`、`resources.qmd`、`slides.qmd`、navbar／sidebar／footer、每週索引、每週頁的 `title`、頁內 TOC 標題、搜尋與 repo-actions（後三項靠 `_quarto.yml` 的 `language:` 覆寫）。中文：每週頁內文與 `subtitle` | 修課學生與外部訪客都要照顧。站名 `自然語言處理與大型語言模型` 不翻 |
| 週次索引 | 英文標題為連結、中文標題為次行（`[…]{.wk-zh}`） | 連結文字與點進去的頁面標題要對得起來 |
| 首頁課程地圖 | 讀 `docs/course-map-en.md`，**不從中文大綱切** | ASCII 圖的對齊靠字元寬度，中英混排無法自動轉換 |
| 表格欄寬 | 用 pipe table 分隔列的**破折號長度**指定比例，不要留 `|---|---|` | Pandoc 依破折號長度分配欄寬；全部等長就是均分，雙語標題那一欄會被擠到不能看 |
| 投影片 | **由授課者自行製作**，AI 不代勞（沿用 Speech-AI 的指示）。`build_weeks.py` 偵測 `slides/wNN.qmd` 存在才注入連結 | 不存在就不給連結，避免死連結 |
...
| 講稿 | 中文，寫在 `::: {.notes}`；`scripts/extract_notes.py` 抽成 `notes/w01.md` 供上課時看 |
...
- `scripts/make_demo_w01.py` — 下載 tinyshakespeare（公版，存到 gitignore 的 `_corpus/`），算出 n=1/2/4/8 的生成、sparsity、bits-per-byte、以及 PPMI+SVD 的 cosine 與最近鄰，寫進 `slides/assets/w01/w01-data.json`（**進版控**，所以不重跑也能 render）。
- `scripts/make_figs_w01.py` — 讀那份 JSON 產生三張 SVG。
...
透過 Cowork 的檔案傳輸寫進 repo 的 SVG 會被加上 C2PA 來源標記（每張多約 8 KB，仍是合法 SVG，能正常 render）。圖是由 `scripts/make_figs_w01.py` 決定性產生的，所以**在 Mac 上重跑一次那支腳本**就會得到乾淨的版本；否則下次重跑時 diff 會很大。
...
6. **行內 `code` 的長識別字會撐破手機版面**。`torch.nn.functional.scaled_dot_product_attention` 在 390px 下把頁面撐寬 89px。上游把行內 code 的 `white-space` 設成 `pre`，**光給 `overflow-wrap: anywhere` 不會生效——必須先把 `white-space` 收回 `normal`**。`styles.scss` 的 `code:not(pre code)` 已處理，`pre` 區塊仍維持 `white-space:pre` + 橫向捲動。**Speech-AI 的 styles.scss 有同一個潛在問題，值得回頭同步。**
